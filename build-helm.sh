...
if [[ -z "${TASK_RUNNER_SOLANA_RPC}" ]]; then
...
for f in "${files[@]}"; do
  ...
  envsubst '$CLUSTER $RPC_URL $WS_URL $TASK_RUNNER_SOLANA_RPC $ORACLE_KEY $HEARTBEAT_INTERVAL $GOOGLE_PAYER_SECRET_PATH $GCP_CONFIG_BUCKET $SERVICE_ACCOUNT_BASE64 $EXTERNAL_IP $PAGERDUTY_EVENT_KEY $GRAFANA_HOSTNAME $GRAFANA_ADMIN_PASSWORD $GRAFANA_TLS_CRT $GRAFANA_TLS_KEY $METRICS_EXPORTER' < "$f" \
